	...
	if (req.buddy())
	{
		...
		for (int j = 0; j < req.neighbours().node_size(); j++)
		{
			...
			if (nodeFrmMsg.nodeid() != "-1")
			{
				new_node = make_shared<Node>(nodeFrmMsg.ip(), nodeFrmMsg.port(), nodeFrmMsg.nodeid());
				//calculate proximity
				ClientDatabase::getInstance().addToNeighhbourSet(new_node);
			}
		}
	}
	if (req.terminal())
...
void PeerMessageHandler::handleShutdownRequest()
{
	...
	for (auto node : leaf_set.first)
	{
		try
		{
			std::string log_msg = "Forwarding Set shutdown request to IP: " + node->getIp() + " Port: " +
		...
		{
			std::string log_msg = "FAILED Forwarding Set shutdown request to IP: " + node->getIp() + " Port: " +
								  node->getPort() + " NodeID: " + node->getNodeID();
			LogHandler::getInstance().logMsg(log_msg);
		}
	}
	LogHandler::getInstance().logMsg("In handle shutdown, After loop CODEA");
	...
	LogHandler::getInstance().logMsg("In handle shutdown, After loop CODEB");
	...
	LogHandler::getInstance().logMsg("In handle shutdown, After loop CODEC");
